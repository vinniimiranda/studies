...
A implementação a seguir mostra uma Hash Table com separate chaining, hash polinomial, resize automático por load factor e operações principais (set/get/delete) com complexidade esperada O(1). Inclui também aplicações clássicas que dependem de hashing: anagramas, agrupamento de anagramas, primeiro caractere não repetido e contagem de subarrays com soma alvo (prefix sum + hash).

### Hash Table com separate chaining + aplicações clássicas
