...
A implementação a seguir reúne, em TypeScript, um exemplo mínimo para cada classe de complexidade discutida no texto — O(1), O(log n), O(n), O(n log n), O(n²), O(2^n) e O(n!) — além de variantes otimizadas via memoization e troca de tempo por memória.

### Exemplos por classe de complexidade
