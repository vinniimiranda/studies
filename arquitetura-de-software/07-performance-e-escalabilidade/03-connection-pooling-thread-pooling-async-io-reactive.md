...
!!! note "Foco deste tópico vs. bloco de Concorrência (14)"
    Aqui o ângulo é **performance e dimensionamento**: como dimensionar pools e por que o modelo assíncrono aumenta o throughput. A **mecânica de concorrência** por trás disso — como threads e troca de contexto funcionam, sincronização, e como `async/await`, Futures/Promises e Reactive Streams operam internamente — vive no bloco 14, para evitar duplicação: ver [Thread pools e tuning](../14-concorrencia-e-paralelismo/08-thread-pools-e-tuning.md) e [Async/await, Futures e Reactive Streams](../14-concorrencia-e-paralelismo/09-async-await-futures-promises-reactive-streams.md). Este arquivo foca o **impacto em latência e throughput**; aquele bloco foca **como funciona**.
