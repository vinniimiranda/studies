...
A implementação a seguir traz uma BST genérica (insert/contains/remove iterativos e recursivos com sucessor in-order), os três traversals em DFS (pre/in/post-order), BFS level-order, cálculo de altura, validação de BST e Lowest Common Ancestor em BST. O mesmo arquivo também contém a implementação de Trie usada na seção de Tries.

### BST + Traversals + Validação + LCA + Trie
